...
export async function statusRoutes(app: FastifyInstance) {
  ...
  app.get('/status/:listId', async (req, reply) => {
    ...
    const doc = await storage.read<any>(`statuslists/${listId}.json`);
  ...
  app.post('/status/init', async (req, reply) => {
    ...
    const doc = { ...base, hash: computeHash(base as any) };
  ...
  app.post('/status/update', async (req, reply) => {
    ...
    const doc = await storage.read<any>(path);
    if (!doc) return reply.code(404).send({ error: 'List not found' });
